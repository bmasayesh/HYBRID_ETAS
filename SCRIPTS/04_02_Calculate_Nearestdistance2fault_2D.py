...
Distdir_all = '../OUTPUTS/DISTANCE-RESULTS/ALL-SUBFAULTS/2D'          ## directory of distance to all subfaults
Distdir_max = '../OUTPUTS/DISTANCE-RESULTS/MAX-SLIP-SUBFAULTS/2D'     ## directory of diatance to subfaults with slip > 15% of max slip 
...
# for nname in range(0, 1):
    ...
    # Observation points:
    gridlat, gridlon = define_grid_circle(Seqdir, dr, name)
    Ntot = len(gridlat)
    ...
    for ns in range(len(slipmodels)):
        ...
        dis1 = np.zeros(Ntot)
        for i in range(Ntot):
            dis1[i] = np.min(dist(gridlat[i], gridlon[i], lats1, lons1))
        
        dis2 = np.zeros(Ntot)
        for i in range(Ntot):
            dis2[i] = np.min(dist(gridlat[i], gridlon[i], lats2, lons2))
            
        outname1 = '%s/distance2fault-%s-dr%.1fkm.out' % (Distdir_all, slipmodels[ns], dr)
        np.savetxt(outname1, np.column_stack((gridlat, gridlon, dis1)), fmt='%f\t%f\t%e', header='lat\tlon\tdist')
        print('\n\t OUTPUT: %s\n' % (outname1))
        
        outname2 = '%s/distance2fault-%s-dr%.1fkm.out' % (Distdir_max, slipmodels[ns], dr)
        np.savetxt(outname2, np.column_stack((gridlat, gridlon, dis2)), fmt='%f\t%f\t%e', header='lat\tlon\tdist')
        print('\n\t OUTPUT: %s\n' % (outname2))
