...
############################# Functions & Classes #############################
def calculate_ETASrate(tmain, Namain, time, rf, fri, df, cf, t, r, mu, Na, c, p, d, q, cnorm):
    R = mu + np.sum(Na * np.power(c + time - t, -p) * LLrecipes.fR(r, d, q, cnorm))
    if time > tmain:
        R += Namain * np.power(c + time - tmain, -p) * (0.34 * LLrecipes.fR(rf, df, q, cf) + 0.66 * fri)
    return R

def update_prob(tmain, Mmain, t, m, ti, rfi, pmaini, dfault, rfault, Vgrid, Ri, mui, Mc, c, p, K, alpha, d0, gamma, q, cnorm, Nind, Nij):
    ...
    for i in range(len(ti)):
        NI = Nind[i]
        nij = Nij[i,:NI]
        rate = calculate_ETASrate(tmain, Namain, ti[i], rfi[i], pmaini[i], dfault, cfault, t[nij], Ri[i, nij], mui[i], Na[nij], c, p, d[nij], q, cnorm[nij])
        prob[i] = mui[i] / rate
    return prob

def LL1value(tmain, Namain, t, ti, Ri, rfi, pmaini, mui, mufac, muV, Na, Zinteg, c, p, d, q, dfault, cnorm, cfault, Nind, Nij, bDT):
    NN = len(ti)
    fac = np.zeros(NN)
    R0 = np.zeros(NN)
    # mutot = mu * V
    for i in range(NN):
        ...
        R0[i] = calculate_ETASrate(tmain, Namain, ti[i], rfi[i], pmaini[i], dfault, cfault, t[nij], Ri[i, nij], mufac*mui[i], Na[nij], c, p, d[nij], q, cnorm[nij])
    LL1 = np.sum(np.log(fac * R0))
    return LL1

def nLLETAS(arguments, rfault, pmaini, Vgrid, tsteps, tmain, Mmain, t, z, m, ti, mi, Ri, rfi, mui, Nback, Mc, T1, T2, Z1, Z2, Nind, Nij, TmaxTrig):
    ...
    LL1 = LL1value(tmain, Namain, t, ti, Ri, rfi, pmaini, mui, mufac, muV, Na, Zinteg, c, p, d, q, dfault, cnorm, cfault, Nind, Nij, bDT)
...
def LLoptimize(x0, rfault, pmaini, Vgrid, tsteps, tmain, Mmain, t, z, m, ti, mi, Ri, rfi, mui, Nbacktot, Mcut, T1, T2, Z1, Z2, Nind, Nij, TmaxTrig):
    res = minimize(nLLETAS, np.sqrt(x0), args=(rfault, pmaini, Vgrid, tsteps, tmain, Mmain, t, z, m, ti, mi, Ri, rfi, mui, Nbacktot, Mcut, T1, T2, Z1, Z2, Nind, Nij, TmaxTrig), method='BFGS', options={'gtol': 1e-01})
...
def setstartparameter(ti, V, T1, T2):
    ...
    # c     = 0.007    # [days]
    # p     = 1.13
    # d0    = 0.026   # ... with d = d0 * 10^(gamma*M) 
    # gamma = 0.32     # L(M) = d0*10^(gamma*M); d0=0.013  gamma=0.5  (Wells & Coppersmith 1994; RLD normal faulting)
    # q     = 0.91     # spatial probability density: f(r) = (q-1)/(pi*D^2) * ( 1 + r^2/D^2 )^(-(1+q))
...
def determine_ETASparameter(latf, lonf, zf, probs, rfault, tmain, Mmain, tall, latall, lonall, zall, mall, Mcut, T1, T2, A, Z1, Z2, stdmin, Nnearest, TmaxTrig):
    '''
    ...
    while dLL > 0.1:
        nround += 1
        mufac, K, alpha, c, p, d0, gamma, q, dfault, b, bDT, LL = LLoptimize(x0, rfault, pmaini, Vgrid, tsteps, tmain, Mmain, t, z, m, ti, mi, Ri, rfi, mui, Nbacktot, Mcut, T1, T2, Z1, Z2, Nind, Nij, TmaxTrig)
        x0 = np.asarray([1.0, K, alpha, c, p, d0, gamma, q, dfault, b, bDT])
        mui *= mufac
        d  = d0 * np.power(10.0, gamma * m)
        cnorm = LLrecipes.determine_normalization_factor_3D(d, q)
        for i in range(10):
            pbacki = update_prob(tmain, Mmain, t, m, ti, rfi, pmaini, dfault, rfault, Vgrid, Ri, mui, Mcut, c, p, K, alpha, d0, gamma, q, cnorm, Nind, Nij) 
            mui, Nbacktot = LLrecipes.calculate_mu(cznorm, dzii, stdzi, pbacki, T1, T2, A)
        if nround == 1:
            dLL = 10.0
        else:
