...
def LL1value(tmain, Namain, t, m, ti, Ri, rfi, pmaini, mu, Na, c, p, d, q, dfault, cfault, Nind, Nij):
    LL1 = 0
    NN = len(ti)
    R = np.zeros(NN)
    for i in range(NN):
        NI = Nind[i]
        nij = Nij[i,:NI]
        R[i] = calculate_ETASrate(tmain, Namain, ti[i], rfi[i], pmaini[i], dfault, cfault, t[nij], Ri[i, nij], mu, Na[nij], c, p, d[nij], q)
    LL1 = np.sum(np.log(R))
    return LL1
...
def nLLETAS(arguments, rfault, LL_GR, A, tmain, Mmain, t, m, ti, Ri, rfi, pmaini, Mc, T1, T2, Nind, Nij, TmaxTrig):
    ...
    LL1 = LL1value(tmain, Namain, t, m, ti, Ri, rfi, pmaini, mu, Na, c, p, d, q, dfault, cfault, Nind, Nij)
...
def LLoptimize(rfault, LL_GR, x0, tmain, Mmain, t, m, ti, Ri, rfi, pmaini, Mcut, T1, T2, A, Nind, Nij, TmaxTrig):
    res = minimize(nLLETAS, np.sqrt(x0), args=(rfault, LL_GR, A, tmain, Mmain, t, m, ti, Ri, rfi, pmaini, Mcut, T1, T2, Nind, Nij, TmaxTrig), method='BFGS', options={'gtol': 1e-01})
...
def determine_ETASparameter(latf, lonf, rfault, probs, tmain, Mmain, tall, latall, lonall, mall, Mcut, T1, T2, A, TmaxTrig):
    ...
    mu, K, alpha, c, p, d0, gamma, q, dfault, LL = LLoptimize(rfault, LL_GR, x0, tmain, Mmain, t, m, ti, Ri, rfi, pmaini, Mcut, T1, T2, A, Nind, Nij, TmaxTrig)  
