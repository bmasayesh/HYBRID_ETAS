...
def update_prob(tmain, Mmain, t, z, m, ti, rfi, pmaini, dfault, rfault, Vgrid, Ri, mui, Mc, c, p, K, alpha, d0, gamma, q, cnorm, Nind, Nij):
...
def LL1value(tmain, Namain, t, z, m, ti, Ri, rfi, pmaini, mui, mufac, Na, c, p, d, q, dfault, cnorm, cfault, Nind, Nij):
    LL1 = 0
    NN = len(ti)
    R = np.zeros(NN)
    for i in range(NN):
        NI = Nind[i]
        nij = Nij[i,:NI]
        R[i] = calculate_ETASrate(tmain, Namain, ti[i], rfi[i], pmaini[i], dfault, cfault, t[nij], Ri[i, nij], mufac*mui[i], Na[nij], c, p, d[nij], q, cnorm[nij])
    LL1 = np.sum(np.log(R))
    return LL1
...
def nLLETAS(arguments, LL_GR, rfault, Vgrid, V, tmain, Mmain, t, z, m, ti, Ri, rfi, pmaini, mui, Nbacktot, Mc, T1, T2, Z1, Z2, Nind, Nij, TmaxTrig):
    ...
    LL1 = LL1value(tmain, Namain, t, z, m, ti, Ri, rfi, pmaini, mui, mufac, Na, c, p, d, q, dfault, cnorm, cfault, Nind, Nij)
    LL2 = LL2valueETAS_2mainshocks(tmain, Namain, t, z, m, ti, Mc, mufac*Nbacktot, K, alpha, c, p, d0, gamma, q, T1, T2, Z1, Z2, TmaxTrig)
    
    # LL2 = LL2valueETAS_2mainshocks(tmain, Namain, t, mufac*Nbacktot, Na*Zinteg, c, p, T1, T2, TmaxTrig)
    LL = LL_GR + LL1 -LL2
    nLL = -LL
    if np.isnan(nLL):
        nLL = 1e10
...
def LLoptimize(LL_GR, x0, rfault, Vgrid, tmain, Mmain, t, z, m, ti, Ri, rfi, pmaini, mui, Nbacktot, Mcut, T1, T2, V, Z1, Z2, Nind, Nij, TmaxTrig):
    res = minimize(nLLETAS, np.sqrt(x0), args=(LL_GR, rfault, Vgrid, V, tmain, Mmain, t, z, m, ti, Ri, rfi, pmaini, mui, Nbacktot, Mcut, T1, T2, Z1, Z2, Nind, Nij, TmaxTrig), method='BFGS', options={'gtol': 1e-01})
...
def determine_ETASparameter(latf, lonf, zf, probs, rfault, tmain, Mmain, tall, latall, lonall, zall, mall, Mcut, T1, T2, A, Z1, Z2, stdzmin, Nnearest, TmaxTrig):
    '''
    ...
    while dLL > 0.1:
        nround += 1
        mufac, K, alpha, c, p, d0, gamma, q, dfault, LL = LLoptimize(LL_GR, x0, rfault, Vgrid, tmain, Mmain, t, z, m, ti, Ri, rfi, pmaini, mui, Nbacktot, Mcut, T1, T2, V, Z1, Z2, Nind, Nij, TmaxTrig)  
        x0 = np.asarray([1.0, K, alpha, c, p, d0, gamma, q, dfault])
        mui *= mufac
        d  = d0 * np.power(10.0, gamma * m)
        cnorm = LLrecipes.determine_normalization_factor_3D(d, q)
        for i in range(10):
            pbacki = update_prob(tmain, Mmain, t, z, m, ti, rfi, pmaini, dfault, rfault, Vgrid, Ri, mui, Mcut, c, p, K, alpha, d0, gamma, q, cnorm, Nind, Nij) 
            mui, Nbacktot = LLrecipes.calculate_mu(cznorm, dzii, stdzi, pbacki, T1, T2, A)
        if nround == 1:
            dLL = 10.0
        else:
