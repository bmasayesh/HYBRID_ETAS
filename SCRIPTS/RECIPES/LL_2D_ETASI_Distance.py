...
def LL1value(tmain, Namain, t, ti, Ri, rfi, mu, muA, Na, c, p, d, q, dfault, cfault, Nind, Nij, bDT):
    NN = len(ti)
    fac = np.zeros(NN)
    R0 = np.zeros(NN)
    for i in range(NN):
        ...
        R0[i] = calculate_ETASrate(tmain, Namain, ti[i], rfi[i], dfault, cfault, t[nij], Ri[i, nij], mu, Na[nij], c, p, d[nij], q)
    LL1 = np.sum(np.log(fac * R0))
    return LL1

def nLLETAS(arguments, rfault, A, tsteps, tmain, Mmain, t, m, ti, mi, Ri, rfi, Mc, T1, T2, Nind, Nij, TmaxTrig):
    ...
    LL1 = LL1value(tmain, Namain, t, ti, Ri, rfi, mu, muA, Na, c, p, d, q, dfault, cfault, Nind, Nij, bDT)
...
def LLoptimize(x0, rfault, tsteps, tmain, Mmain, t, m, ti, mi, Ri, rfi, Mcut, T1, T2, A, Nind, Nij, TmaxTrig):  
    res = minimize(nLLETAS, np.sqrt(x0), args=(rfault, A, tsteps, tmain, Mmain, t, m, ti, mi, Ri, rfi, Mcut, T1, T2, Nind, Nij, TmaxTrig), method='BFGS', options={'gtol': 1e-01})
...
def setstartparameter(A):
    ...
    p     = 0.93
    d0    = 0.013   # ... with d = d0 * 10^(gamma*M) 
    gamma = 0.35     # L(M) = d0*10^(gamma*M); d0=0.013  gamma=0.5  (Wells & Coppersmith 1994; RLD normal faulting)
    q     = 0.46     # spatial probability density: f(r) = (q-1)/(pi*D^2) * ( 1 + r^2/D^2 )^(-(1+q))
    dfault = 0.9    # for distance-to-fault decay of mainshock-triggered events
    b     = 1.0
    bDT   = 100.0 / (24 * 60 * 60.0)   #  blind time
...
def determine_ETASparameter(latf, lonf, rfault, tmain, Mmain, tall, latall, lonall, mall, Mcut, T1, T2, A, TmaxTrig):
    ...
    mu, K, alpha, c, p, d0, gamma, q, dfault, b, bDT, LL = LLoptimize(x0, rfault, tsteps, tmain, Mmain, t, m, ti, mi, Ri, rfi, Mcut, T1, T2, A, Nind, Nij, TmaxTrig)
    tfac = 24*60.0
