...
'''
This code estimates standard 3D ETAS parameters for 6 large earthquakes in California. 
...
import LL_3D_ETAS as LLETASUNIBA      ## importing function for parameter estimation   

############################# Functions & Classes #############################
def estimate_parameters_3D_ETAS(Paradir, name, t, lat, lon, z, m, Mcut, T1, T2, Z1, Z2, stdmin, Nnearest, TmaxTrig, A):
    mutot, K, alpha, c, p, d0, gamma, q, b, LLvalue = LLETASUNIBA.determine_ETASparameter(t, lat, lon, z, m, Mcut, T1, T2, Z1, Z2, stdmin, Nnearest, TmaxTrig, A)
    suff = 'Standard-3D_ETAS-%s-Mcut%.2f-T%.1f-%.1f' % (name, Mcut, T1, T2)
    # OUTPUTS:
...
Paradir = '../OUTPUTS/PARAMETERS/3D ETAS/ETAS'   ## directory of parameters 
...
### ===================== 3D ETAS parameter esimation =========================
for i in range(4, 5):      ## Here we just chose Baja California
    name = names[i]
    print('\n\t Estimation of 3D ETAS parameters for %s sequence' % (name))
    ...
    z = data[1:, 4]
    m = data[1:, 5]
    estimate_parameters_3D_ETAS(Paradir, name, t, lat, lon, z, m, Mcut, T1, T2, Z1, Z2, stdmin, Nnearest, TmaxTrig, A)
