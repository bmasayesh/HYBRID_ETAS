...
import LL_2D_ETASI_Distance as LLETASI_R
...
def estimate_parameters_2D_ETASI_DIST(Paradir, name, latf, lonf, rfault, tmain, Mmain, t, lat, lon, m, Mcut, T1, T2, A, TmaxTrig):
    mutot, K, alpha, c, p, d0, gamma, q, dfault, b, bDT, LLvalue = LLETASI_R.determine_ETASparameter(latf, lonf, rfault, tmain, Mmain, t, lat, lon, m, Mcut, T1, T2, A, TmaxTrig)
    suff = '2D_ETASI-distance-to-fault-%s-Mcut%.2f-T%.1f-%.2f' % (name, Mcut, T1, T2)
    # OUTPUTS:
...
Paradir = '../OUTPUTS/PARAMETERS/2D ETAS/ETASI_R'          ## directory of parameters 
...
###======2D ETASI+Distance parameter estimation (using mainshock stress)=======
'''
...
for i in range(4, 5):          ## Here we just chose Baja California 
    ...
    print('\n\t Estimation of 2D ETASI + R parameter for %s sequence\n' % (name))
    ...
    lon = data[ind, 3]
    m = data[ind, 5]
    estimate_parameters_2D_ETASI_DIST(Paradir, name, lati, loni, dist, tmain, Mmain, t, lat, lon, m, Mcut, T1, T2, A, TmaxTrig)
