...
'''
This code estimates 3D ETASI parameters by considering stress scalars from mainshocks for 
...
import LL_3D_ETASI_Stress as LLETASI_stress
day2sec = 24 * 60 * 60.0
...
def estimate_parameters_ETASI_3D_stress(Paradir, name, stressvalue, dr, lati, loni, zi, probi, tmain, Mmain, t, lat, lon, z, m, Mcut, T1, T2, A, Z1, Z2, stdmin, Nnearest, TmaxTrig):
    mutot, K, alpha, c, p, d0, gamma, q, b, bDT, LLvalue =  LLETASI_stress.determine_ETASparameter(dr, lati, loni, zi, probi, tmain, Mmain, t, lat, lon, z, m, Mcut, T1, T2, A, Z1, Z2, stdmin, Nnearest, TmaxTrig)
    suff = '3D_ETASI-%s-%s-Mcut%.2f-T%.1f-%.2f' % (stressvalue, name, Mcut, T1, T2)
    # OUTPUTS:
...
###====== 3D ETASI+Stress parameter estimation (using mainshock stress)========
'''
...
for i in range(4, 5):            ## Here we just chose Baja California     
    ...
    print('\n\t Estimation of 3D ETASI + %s parameter for %s sequence\n' % (stressvalue, name))
    ...
    z = data[1:, 4]
    m = data[1:, 5]
    estimate_parameters_ETASI_3D_stress(Paradir, name, stressvalue, dr, lati, loni, zi, probi, tmain, Mmain, t, lat, lon, z, m, Mcut, T1, T2, A, Z1, Z2, stdmin, Nnearest, TmaxTrig)
