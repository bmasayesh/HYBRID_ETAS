...
import LL_3D_ETAS_Distance as LLETAS_R
...
def estimate_parameters_ETAS_3D_distance_to_fault(Paradir, name, latf, lonf, zf, rfault, tmain, Mmain, t, lat, lon, z, m, Mcut, T1, T2, A, Z1, Z2, stdmin, Nnearest, TmaxTrig):

    mutot, K, alpha, c, p, d0, gamma, q, dfault, b, LLvalue = LLETAS_R.determine_ETASparameter(latf, lonf, zf, rfault, tmain, Mmain, t, lat, lon, z, m, Mcut, T1, T2, A, Z1, Z2, stdmin, Nnearest, TmaxTrig)
    suff = '3D_ETAS-distance-to-fault-%s-Mcut%.2f-T%.1f-%.2f' % (name, Mcut, T1, T2)
    # OUTPUTS:
...
Paradir = '../OUTPUTS/PARAMETERS/3D ETAS/ETAS_R'      ## directory of parameters 
...
## =========== 3D ETAS+Distance to fault plane parameter estimation ===========
for i in range(4, 5):         ## Here we just chose Baja California          
    ...
    print('\n\t Estimation of 3D ETAS + R parameter for %s sequence' % (name))
    ...
    z = data[1:, 4]
    m = data[1:, 5]
    estimate_parameters_ETAS_3D_distance_to_fault(Paradir, name, lati, loni, zi, dist, tmain, Mmain, t, lat, lon, z, m, Mcut, T1, T2, A, Z1, Z2, stdmin, Nnearest, TmaxTrig)
