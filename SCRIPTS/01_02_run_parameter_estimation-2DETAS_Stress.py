...
'''
This code estimates 2D ETAS parameters by considering stress scalars from mainshocks for 
...
import LL_2D_ETAS_Stress as LLETAS_stress
...
def estimate_parameters_ETAS_3D_stress(Paradir, name, stressvalue, lati, loni, probi, tmain, Mmain, t, lat, lon, m, Mcut, T1, T2, A, TmaxTrig):
    mu, K, alpha, c, p, d0, gamma, q, b, LLvalue =  LLETAS_stress.determine_ETASparameter(lati, loni, probi, tmain, Mmain, t, lat, lon, m, Mcut, T1, T2, A, TmaxTrig)
    suff = '2D_ETAS-%s-%s-Mcut%.2f-T%.1f-%.2f' % (stressvalue, name, Mcut, T1, T2)
    # OUTPUTS:
    outname = '%s/ETAS_%s/parameter-%s.out' % (Paradir, stressvalue, suff)
    f = open(outname, 'w')
    f.write('#     mutot            K             alpha         c_[days]            p            d0_[km]          gamma             q            b            LL\n')
    f.write('%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n' % (mu, K, alpha, c, p, d0, gamma, q, b, LLvalue))
    f.close()
    print('\n\t OUTPUT: %s' % (outname))
    return
...
for i in range(4, 5):       ## Here we just chose Baja California
    ...
    print('\n\t Estimation of 2D ETAS + %s parameter for %s sequence\n' % (stressvalue, name))
    ...
    lon = data[ind, 3]
    m = data[ind, 5]
    estimate_parameters_ETAS_3D_stress(Paradir, name, stressvalue, lati, loni, probi, tmain, Mmain, t, lat, lon, m, Mcut, T1, T2, A, TmaxTrig)
